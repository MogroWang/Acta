...
const dictionaries = {
  zh: {
    ...
    syncCopy: '选择 OneDrive、Dropbox、坚果云或其他网盘的本地同步文件夹。Acta 会在其中读写一个通用数据文件。',
  ...
  en: {
    ...
    syncCopy: 'Choose a local sync folder from OneDrive, Dropbox or another cloud drive. Acta reads and writes one portable data file there.',
...
function translateStaticUI() {
  ...
  $('#languageLabel').textContent = settings.language === 'zh' ? '中' : 'EN';
}
...
function renderSidebar() {
  ...
  $('#folderNav').innerHTML = library.folders.map(folder => {
    ...
    return `<button data-view="folder:${escapeHTML(folder.id)}" class="${currentView === `folder:${folder.id}` ? 'active' : ''}">
      <span><i class="folder-dot" style="background:${escapeHTML(folder.color)};color:${escapeHTML(folder.color)}"></i><span>${escapeHTML(folderName(folder))}</span></span><em>${count}</em>
    </button>`;
  }).join('');

  $('#syncSubtitle').textContent = settings.syncFolder ? t('syncReady') : t('notConfigured');
  if (settings.syncFolder) $('#syncSubtitle').title = settings.syncFolder;
}
...
function classificationField(item) {
  return `<div class="meta-field classification-field">
    ...
    <div class="classification-controls">
      ...
      <input id="classificationTags" value="${escapeHTML((item.tags || []).join(', '))}" placeholder="${t('commaTags')}" />
    </div>
  </div>`;
}
...
function noteEditor(item) {
  ...
  return `<article class="editor-wrap note-editor" data-editor-id="${escapeHTML(item.id)}">
    ...
    <div class="editor-subline"><span><svg><use href="#i-clock"/></svg>${t('modified')} ${formatDate(item.updatedAt)}</span><span><svg><use href="#i-tag"/></svg>${(item.tags || []).map(escapeHTML).join(' · ') || t('tags')}</span></div>
...
function todoEditor(item) {
  ...
  return `<article class="editor-wrap todo-editor" data-editor-id="${escapeHTML(item.id)}">
    ...
    <div class="todo-meta-grid">
      <div class="meta-field"><label><svg><use href="#i-calendar"/></svg>${t('dueDate')}</label><input id="dueInput" type="date" value="${escapeHTML(item.due || '')}" /></div>
      <div class="meta-field"><label><svg><use href="#i-spark"/></svg>${t('priority')}</label><div class="priority-options">
        ${['high','medium','low'].map(value => `<button data-priority="${value}" class="${item.priority === value ? 'active' : ''}">${t(value)}</button>`).join('')}
      </div></div>
      ${classificationField(item)}
    </div>
    ${linkedItemsSection(item)}
...
function bindTodoEditor(item) {
  $('#dueInput').addEventListener('change', event => { item.due = event.target.value; touchItem(item); renderList(); });
  $$('.priority-options button').forEach(button => button.addEventListener('click', () => {
    item.priority = button.dataset.priority; touchItem(item);
  ...
  $('#addTask').addEventListener('click', () => addTask(item));
  $('#todoNotes').addEventListener('input', event => { item.notes = event.target.textContent; touchItem(item); updateCard(item); });
}
...
function createItem(type) {
  ...
  const base = { id: uid(), type, folderId: getFolder(currentFolder) ? currentFolder : library.folders[0]?.id, title: type === 'note' ? t('untitledNote') : t('untitledTodo'), tags: [], linkedIds: [], createdAt: now, updatedAt: now };
  const item = type === 'note'
    ? { ...base, body: '<p><br></p>' }
    : { ...base, due: todayISO(), priority: 'medium', notes: '', tasks: [{ id: uid(), text: '', done: false }], completed: false };
  ...
  requestAnimationFrame(() => { $('#editorTitle')?.select(); });
}
...
function bindShell() {
  ...
  $('#languageButton').addEventListener('click', () => {
    ...
    persist(); renderAll(); showToast(t('languageChanged'));
  });
  $('#listMenu').addEventListener('click', openSyncModal);
  document.addEventListener('keydown', event => {
    ...
    if (event.key === 'Escape') { $('#createMenu').classList.remove('open'); closeSyncModal(); }
  });
}
...
function updateSyncModal() {
  ...
  $('#folderPath').textContent = settings.syncFolder || t('noFolder');
  $('#folderPath').title = settings.syncFolder || '';
  $('#folderPickerTitle').textContent = settings.syncFolder ? t('syncReady') : t('chooseFolder');
  $('#downloadButton').disabled = !settings.syncFolder;
  $('#uploadButton').disabled = !settings.syncFolder;
}

function syncStatus(message, state = '') {
...
function bindSync() {
  ...
  $$('[data-close-modal]').forEach(button => button.addEventListener('click', closeSyncModal));
  $('#syncModal').addEventListener('click', event => { if (event.target.id === 'syncModal') closeSyncModal(); });
  $('#chooseFolderButton').addEventListener('click', async () => {
    const bridge = getSyncBridge();
    if (!bridge) { syncStatus('Sync API unavailable', 'error'); return; }
    const folder = await bridge.chooseSyncFolder();
    if (!folder) return;
    settings.syncFolder = folder; persist(); updateSyncModal(); renderSidebar(); syncStatus(t('safeLocal'));
  });
  $('#uploadButton').addEventListener('click', async () => {
    if (!settings.syncFolder) { showToast(t('chooseFirst')); return; }
    syncStatus(t('syncWorking'), 'working');
    try {
      const result = await getSyncBridge().uploadLibrary(settings.syncFolder, library);
      settings.lastSyncedAt = result.syncedAt; persist(); syncStatus(`${t('uploadDone')} · ${formatDate(result.syncedAt)}`); showToast(t('uploadDone'));
    } catch (error) { syncStatus(`${t('invalidData')}: ${error.message}`, 'error'); }
  });
  $('#downloadButton').addEventListener('click', async () => {
    if (!settings.syncFolder) { showToast(t('chooseFirst')); return; }
    syncStatus(t('syncWorking'), 'working');
    try {
      const result = await getSyncBridge().downloadLibrary(settings.syncFolder);
      if (!Array.isArray(result.library.items) || !Array.isArray(result.library.folders)) throw new Error('Invalid library');
      library = normalizeLibrary(result.library); selectedId = library.items[0]?.id || null; settings.lastSyncedAt = result.syncedAt;
      localStorage.setItem(STORAGE_KEY, JSON.stringify(library)); persist(); renderAll(); syncStatus(`${t('downloadDone')} · ${formatDate(result.syncedAt)}`); showToast(t('downloadDone'));
    } catch (error) { syncStatus(`${t('invalidData')}: ${error.message}`, 'error'); }
  });
}
